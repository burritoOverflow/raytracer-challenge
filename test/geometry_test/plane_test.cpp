...
TEST(PlaneTest, TestNormalOfPlaneIsConstantEverywhere) {
    geometry::Plane p{};
    const auto n1 = p.LocalNormalAt({0, 0, 0});
    const auto n2 = p.LocalNormalAt({10, 0, -10});
    const auto n3 = p.LocalNormalAt({-5, 0, 150});
...
TEST(PlaneTest, TestIntersectWithRayParallelToPlane) {
    geometry::Plane p{};
    const commontypes::Ray r{{0, 10, 0}, {0, 0, 1}};
...
TEST(PlaneTest, TestIntersectCoplanarRay) {
    ...
    geometry::Plane p{};
    const commontypes::Ray r{{0, 0, 0}, {0, 0, 1}};
    const auto xs = p.LocalIntersect(r);
    ASSERT_TRUE(xs.empty());
}
...
TEST(PlaneTest, TestRayIntersectingPlaneFromAbove) {
    geometry::Plane p{};
    const commontypes::Ray r{{0, 1, 0}, {0, -1, 0}};
    const auto xs = p.LocalIntersect(r);
